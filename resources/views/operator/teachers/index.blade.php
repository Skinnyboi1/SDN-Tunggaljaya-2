...
<div class="space-y-8">
        ...
        <form action="{{ route('operator.teachers.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="name" required placeholder="Contoh: Budi Santoso, S.Pd." class="w-full px-3.5 py-2.5 bg-[#9e6f54] border border-[#835841] rounded-xl text-xs text-white placeholder-primary/60 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary">
                </div>

                <div>
                    <label class="block text-xs font-bold text-primary mb-1">NIP (Opsional)</label>
                    <input type="text" name="nip" placeholder="1990..." class="w-full px-3.5 py-2.5 bg-[#9e6f54] border border-[#835841] rounded-xl text-xs text-white placeholder-primary/60 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary">
                </div>

                <div>
                    <label class="block text-xs font-bold text-primary mb-1">Jabatan / Peran</label>
                    <input type="text" name="position" required placeholder="Guru Kelas / PJOK" class="w-full px-3.5 py-2.5 bg-[#9e6f54] border border-[#835841] rounded-xl text-xs text-white placeholder-primary/60 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-12 gap-4 items-center">
                <div class="sm:col-span-8">
                    ...
                    <div id="dropzone-teacher" 
                        ...
                        <div id="preview-box-teacher" class="hidden flex items-center gap-3 w-full">
                            <img id="preview-img-teacher" src="#" alt="Preview" class="w-12 h-12 rounded-xl object-cover border-2 border-primary shadow">
                            ...
                            <button type="button" id="remove-btn-teacher" class="p-1.5 rounded-lg bg-rose-600/40 hover:bg-rose-600 text-rose-100 transition">
                ...
                <div class="sm:col-span-4 space-y-3">
                    <div>
                        ...
                        <input type="text" name="photo" placeholder="https://..." class="w-full px-3 py-2 bg-[#9e6f54] border border-[#835841] rounded-xl text-xs text-white placeholder-primary/50 focus:outline-none focus:border-primary">
                    </div>
    ...
    <div class="bg-secondary text-white rounded-2xl border border-[#9e6f54] shadow-xl overflow-hidden">
        <div class="p-6 border-b border-[#9e6f54] flex items-center justify-between">
            <h3 class="text-base font-bold text-white">Daftar Guru & Staf ({{ count($teachers) }})</h3>
        </div>
                ...
                <tbody class="divide-y divide-[#835841]">
                    @forelse($teachers as $teacher)
                        <tr class="hover:bg-[#9e6f54]/50 transition">
                            ...
                            <td class="px-6 py-3 font-bold text-white">{{ $teacher->name }}</td>
                            <td class="px-6 py-3 font-mono text-primary-200">{{ $teacher->nip ?? '-' }}</td>
                            <td class="px-6 py-3 font-semibold text-primary">{{ $teacher->position }}</td>
